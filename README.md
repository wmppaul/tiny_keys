...
- Audio engine, sampled piano, and waveform playback: [`TinyKeys/Audio/SynthEngine.swift`](./TinyKeys/Audio/SynthEngine.swift)

## Sounds

Tiny Keys now uses a hybrid sound setup:

- `Piano` is loaded through `AVAudioUnitSampler` from the bundled `UprightPianoKW-small-20190703.sf2` sound font in [`TinyKeys/Resources/SoundFonts/UprightPianoKW-small-20190703.sf2`](./TinyKeys/Resources/SoundFonts/UprightPianoKW-small-20190703.sf2)
- `Sine`, `Square`, and `Triangle` are generated by the lightweight built-in waveform engine in [`TinyKeys/Audio/SynthEngine.swift`](./TinyKeys/Audio/SynthEngine.swift)
